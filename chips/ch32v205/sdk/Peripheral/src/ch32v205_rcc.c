...
#define CFGR0_PLL_Mask              ((uint32_t)0xFF80FFFF)
#define CFGR0_PLLMull_Mask          ((uint32_t)0x007C0000)
 ...
 *             RCC_USBCLKSource_PLLCLK_Div4 - PLL clock selected as USB clock source(192MHz).
 *             RCC_USBCLKSource_PLLCLK_Div1_5 - PLL clock selected as USB clock source(72MHz).
 *             RCC_USBCLKSource_PLLCLK_Div2_5 - PLL clock selected as USB clock source(120MHz).
 *             RCC_USBCLKSource_PLLCLK_Div3_5 - PLL clock selected as USB clock source(168MHz).
 *
 * @return  none
 */
void RCC_USBCLKConfig(uint32_t RCC_USBCLKSource)
{
    RCC->CFGR0 &= ~((uint32_t)(7 << 28));
 ...
 *          RCC_ADC_USBHS_Div5 - ADC clock = USBHSCLK/5.
 *          RCC_ADC_USBHS_Div6 - ADC clock = USBHSCLK/6.
 *
 * @return  none
 */
...
{
    uint32_t tmp = 0, pllmull = 0, pllsource = 0, presc = 0, tmp1 = 0;
    ...
    {
      RCC_Clocks->ADCCLK_Frequency = 96000000;
    }
    else if(tmp1 == 0x03)
    {
      RCC_Clocks->ADCCLK_Frequency = 80000000;
    }
}
 ...
 *            RCC_PB1Periph_USART6
 *            RCC_PB1Periph_USART7
 *            RCC_PB1Periph_USART8
 *            RCC_PB1Periph_I2C1.
 *            RCC_PB1Periph_I2C2.
 *            RCC_PB1Periph_CAN1.
 *            RCC_PB1Periph_BKP.
 ...
 *            RCC_USBHSPLL_IN_Divx - x can select(1~8)
 *
 * @return  none
 */
 ...
 *
 * @param   RCC_USBHSPLLRefer - the USBSS PLL refer clock.
 *            RCC_USBHSPLLRefer_5M - USBSS PLL Refer clock 5M
 *            RCC_USBHSPLLRefer_8M - USBSS PLL Refer clock 8M
 *            RCC_USBHSPLLRefer_20M - USBSS PLL Refer clock 20M
 *            RCC_USBHSPLLRefer_24M - USBSS PLL Refer clock 24M
 *
 * @return  none
 */
